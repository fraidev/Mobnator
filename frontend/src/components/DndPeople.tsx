...
const DndPeople = forwardRef((prop, ref: Ref<DndPeopleRef>) => {
  ...
  const renderCard = (card: { id: number; text: string }, index: number) => {
    return <DndPersonCard key={card.id} index={index} id={card.id} text={card.text} moveCard={moveCard} />;
  };
  return <DndProvider backend={Backend}>{<div style={style}>{cards.map((card, i) => renderCard(card, i))}</div>}</DndProvider>;
});
